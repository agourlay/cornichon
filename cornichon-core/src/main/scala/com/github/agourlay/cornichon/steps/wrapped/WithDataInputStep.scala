...
case class WithDataInputStep(nested: List[Step], where: String, rawJson: Boolean = false) extends WrapperStep {
  ...
  override val stateUpdate: StepState = StateT { runState =>
    ...
    runState.scenarioContext
      .fillPlaceholders(where)
      ...
      .fold(
        ...
        parsedTable => {
          val inputs = parsedTable.map { line =>
            line.toIterable.map { case (key, json) => (key, if (rawJson) json.noSpacesSortKeys else CornichonJson.jsonStringValue(json)) }.toList
          }

          runInputs(inputs, runState.nestedContext).timed
            .map { case (executionTime, (inputsState, inputsRes)) =>
              val initialDepth = runState.depth
              val (logStack, res) = inputsRes match {
                case Right(_) =>
                  val wrappedLogStack =
                    SuccessLogInstruction("With data input succeeded for all inputs", initialDepth, Some(executionTime)) +: inputsState.logStack :+ successTitleLog(
                      initialDepth
                    )
                  (wrappedLogStack, rightDone)
                case Left((failedInputs, failedStep)) =>
                  val wrappedLogStack =
                    FailureLogInstruction("With data input failed for one input", initialDepth, Some(executionTime)) +: inputsState.logStack :+ failedTitleLog(
                      initialDepth
                    )
