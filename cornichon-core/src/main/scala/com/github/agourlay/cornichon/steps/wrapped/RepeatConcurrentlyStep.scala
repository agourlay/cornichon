      ...
      .flatMap { case (executionTime, results) =>
        if (results.size != times) {
          ...
          failedStepRuns.headOption.fold[IO[(RunState, Either[FailedStep, Done])]] {
            ...
            val wrappedLogStack =
              SuccessLogInstruction(s"Repeat concurrently block succeeded", initialDepth, Some(executionTime)) +: firstStateLog :+ successTitleLog(initialDepth)
