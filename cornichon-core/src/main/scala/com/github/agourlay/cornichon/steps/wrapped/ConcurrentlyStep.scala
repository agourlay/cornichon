      ...
      .flatMap { case (executionTime, results) =>
        if (results.size != nested.size) {
          ...
          failedStepRuns.headOption.fold[IO[(RunState, Either[FailedStep, Done])]] {
            ...
            val wrappedLogStack =
              SuccessLogInstruction(s"Concurrently block succeeded", initialDepth, Some(executionTime)) +: firstStateLog :+ successTitleLog(initialDepth)
