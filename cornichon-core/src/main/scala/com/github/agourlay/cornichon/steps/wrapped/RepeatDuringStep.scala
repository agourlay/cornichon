...
case class RepeatDuringStep(nested: List[Step], duration: FiniteDuration) extends WrapperStep {
  ...
  override val stateUpdate: StepState = StateT { runState =>
    ...
    repeatStepsDuring(runState.nestedContext, duration, 0).timed
        ...
        val (logStack, res) = report.fold(
          failedStep => {
            val wrappedLogStack = FailureLogInstruction(
              s"Repeat block during '$duration' failed after being retried '$retries' times",
              initialDepth,
              Some(executionTime)
            ) +: repeatedRunState.logStack :+ failedTitleLog(initialDepth)
          ...
          _ => {
            val wrappedLogStack = SuccessLogInstruction(
              s"Repeat block during '$duration' succeeded after '$retries' retries",
              initialDepth,
              Some(executionTime)
            ) +: repeatedRunState.logStack :+ successTitleLog(initialDepth)
