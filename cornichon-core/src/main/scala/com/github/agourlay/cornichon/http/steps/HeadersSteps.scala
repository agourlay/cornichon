      ...
      action = sc =>
        Assertion.either {
          for {
            ...
            expectedWithResolvedPlaceholders <- traverseIL(expected.iterator) { case (name, value) =>
      ...
      action = sc =>
        Assertion.either {
          for {
            ...
            sessionHeadersValue <- decodeSessionHeaders(sessionHeaders)
            lowerCasedActual = sessionHeadersValue.map { case (name, value) => name.toLowerCase -> value }
            elementsWithResolvedPlaceholders <- traverseIL(elements.iterator) { case (name, value) =>
                                                  sc.fillPlaceholders(value).map(v => (name, v))
                                                }
