...
case class RetryMaxStep(nested: List[Step], limit: Int) extends WrapperStep {
  ...
  override val stateUpdate: StepState = StateT { runState =>
    ...
    retryMaxSteps(runState.nestedContext, limit, 0).timed
      .map { case (executionTime, run) =>
        ...
        val (logStack, res) = report.fold(
          failedStep => {
            val wrappedLogStack =
              FailureLogInstruction(s"RetryMax block with limit '$limit' failed", depth, Some(executionTime)) +: retriedState.logStack :+ failedTitleLog(depth)
          ...
          _ => {
            val wrappedLogStack = SuccessLogInstruction(
              s"RetryMax block with limit '$limit' succeeded after '$retries' retries",
              depth,
              Some(executionTime)
            ) +: retriedState.logStack :+ successTitleLog(depth)
