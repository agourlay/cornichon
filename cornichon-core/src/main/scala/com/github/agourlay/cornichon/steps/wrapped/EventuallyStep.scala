...
case class EventuallyStep(nested: List[Step], conf: EventuallyConf) extends WrapperStep {
  ...
  override val stateUpdate: StepState = StateT { runState =>
    ...
    retryEventuallySteps(runState.nestedContext, conf, 0, None)
      ...
      .timed
      .map { case (executionTime, (retries, retriedRunState, report)) =>
        ...
        val wrappedLogStack = report match {
          ...
          case _ =>
            SuccessLogInstruction(
              s"Eventually block succeeded after '$retries' retries",
              initialDepth,
              Some(executionTime)
            ) +: retriedRunState.logStack :+ successTitleLog(initialDepth)
        }
