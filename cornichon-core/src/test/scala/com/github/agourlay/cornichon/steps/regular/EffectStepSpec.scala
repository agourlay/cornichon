...
class EffectStepSpec extends FunSuite with CommonTestSuite {
  ...
  test("CatsEffectStep Async - return error if an Effect step throw an exception") {
    val step = CEffectStep(title = "buggy effect", _ => throwExceptionWithStackTrace())
    val s = Scenario("scenario with broken effect step", step :: Nil)
    val res = awaitIO(ScenarioRunner.runScenario(Session.newEmpty)(s))
    assert(!res.isSuccess)
  }

  test("CatsEffectStep Async - return error if an Effect step for IO.raiseError") {
    val step = CEffectStep(title = "buggy effect", _ => IO.raiseError(new RuntimeException("boom") with NoStackTrace))
    val s = Scenario("scenario with broken effect step", step :: Nil)
    val res = awaitIO(ScenarioRunner.runScenario(Session.newEmpty)(s))
    assert(!res.isSuccess)
  }
