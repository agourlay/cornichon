...
class CornichonJsonProperties extends Properties("CornichonJson") with CornichonJson with ArbitraryInstances {
  ...
  property("parseJson Boolean") = forAll { (bool: Boolean) =>
    parseDslJson(bool) == Right(Json.fromBoolean(bool))
  }

  property("parseJson Int") = forAll { (int: Int) =>
    parseDslJson(int) == Right(Json.fromInt(int))
  }

  property("parseJson alphanumeric String") = forAll(Gen.alphaNumStr) { (s: String) =>
  ...
  property("parseJson alphanumeric String conserves spaces") = forAll(Gen.alphaNumStr) { (s: String) =>
    val decorated = s"  $s  "
    parseDslJson(decorated) == Right(Json.fromString(decorated))
  }

  property("parseJson Long") = forAll { (long: Long) =>
    parseDslJson(long) == Right(Json.fromLong(long))
  }

  property("parseJson Double") = forAll { (double: Double) =>
    parseDslJson(double) == Right(Json.fromDoubleOrNull(double))
  }

  property("parseJson BigDecimal") = forAll { (bigDec: BigDecimal) =>
  ...
  property("findAllContainingValue find key in any JsonObject") = {
    ...
    forAll { (jos: List[JsonObject]) =>
      ...
      val (value, path) = findAllPathWithStringValue(Set("target value"), json).head
      value == "target value" && path.run(json).contains(targetValue)
    }
  }

  property("whitelisting on identical JSON has no effect") = forAll { (json: Json) =>
    whitelistingValue(json, json) == Right(json)
  }

  property("whitelisting improper subset with single added key") = forAll { (jsonObj: JsonObject) =>
  ...
  property("whitelisting arrays with single missing key on each element") = forAll { (jos: List[JsonObject]) =>
    // ==> did not work
    if (jos.isEmpty) {
      true
    } else {
