...
class JsonPathParserProperties extends Properties("JsonPathParser") {
  ...
  property("parse JsonPath containing a field without index") = forAll(fieldGen) { field =>
    JsonPathParser.parseJsonPath(field) == Right(Vector(FieldSelection(field)))
  }

  property("parse JsonPath containing a field with index") = forAll(fieldGen, indexGen) { (field, index) =>
    JsonPathParser.parseJsonPath(s"$field[$index]") == Right(Vector(ArrayFieldSelection(field, index)))
  }

  property("parse JsonPath containing two fields without index") = forAll(fieldGen, fieldGen) { (field1, field2) =>
    JsonPathParser.parseJsonPath(s"$field1.$field2") == Right(Vector(FieldSelection(field1), FieldSelection(field2)))
  }

  property("parse JsonPath containing a field with projection") = forAll(fieldGen) { field =>
    JsonPathParser.parseJsonPath(s"$field[*]") == Right(Vector(ArrayFieldProjection(field)))
  }

  property("parse JsonPath with key containing a dot without index") = forAll(fieldGen, fieldGen, fieldGen) { (field1, field2, field3) =>
  ...
  property("parse JsonPath with key containing a dot with index") = forAll(fieldGen, fieldGen, fieldGen, indexGen) { (field1, field2, field3, index) =>
    val composedPath = s"$field1.$field2"
  ...
  property("return error if it starts with '.'") = forAll(fieldGen) { field =>
    JsonPathParser.parseJsonPath(s".$field.").isLeft
  }

  property("parse JsonPath containing a missing field") = forAll(fieldGen) { field =>
    JsonPathParser.parseJsonPath(s"$field.").isLeft
  }

  property("parse JsonPath containing a broken index bracket") = forAll(fieldGen, indexGen) { (field, index) =>
