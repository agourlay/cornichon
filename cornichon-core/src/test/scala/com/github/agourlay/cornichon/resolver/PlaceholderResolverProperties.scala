...
class PlaceholderResolverProperties extends Properties("PlaceholderResolver") {
  ...
  property("findPlaceholders finds placeholder in content solely containing a placeholder without index") = forAll(keyGen) { key =>
    findPlaceholders(s"<$key>") == Right(Vector(Placeholder(key, None)))
  }

  property("findPlaceholders find placeholder in content solely containing a placeholder with index") = forAll(keyGen, indexGen) { (key, index) =>
    findPlaceholders(s"<$key[$index]>") == Right(Vector(Placeholder(key, Some(index))))
  }

  property("findPlaceholders find placeholder in content starting with whitespace and containing a placeholder") = forAll(keyGen) { key =>
    findPlaceholders(s" <$key>") == Right(Vector(Placeholder(key, None)))
  }

  property("findPlaceholders find placeholder in content starting with 2 whitespaces and containing a placeholder") = forAll(keyGen) { key =>
    findPlaceholders(s"  <$key>") == Right(Vector(Placeholder(key, None)))
  }

  property("findPlaceholders find placeholder in content finishing with whitespace and containing a placeholder") = forAll(keyGen) { key =>
    findPlaceholders(s"<$key> ") == Right(Vector(Placeholder(key, None)))
  }

  property("findPlaceholders find placeholder in content finishing with 2 whitespaces and containing a placeholder") = forAll(keyGen) { key =>
    findPlaceholders(s"<$key>  ") == Right(Vector(Placeholder(key, None)))
  }

  property("findPlaceholders find placeholder in random content containing a placeholder with index") = forAll(keyGen, indexGen, Gen.alphaStr) { (key, index, content) =>
  ...
  property("fillPlaceholders replaces a single string") = forAll(keyGen, valueGen) { (ph, value) =>
  ...
  property("fillPlaceholders take the first value in session if index = 0") = forAll(keyGen, valueGen, valueGen) { (key, firstValue, secondValue) =>
  ...
  property("fillPlaceholders returns ResolverError if placeholder not found") = forAll(keyGen, valueGen) { (key, value) =>
  ...
  property("fillPlaceholders resolves two placeholders") = forAll(keyGen, valueGen, valueGen) { (k1, v1, v2) =>
    val k2 = k1 + "42"
  ...
  property("fillPlaceholders is not be confused by unclosed markup used in a math context") = forAll(keyGen, valueGen) { (key, value) =>
    val session = Session.newEmpty.addValueUnsafe(key, value)
    val content = s"3.15 > <$key>"
    PlaceholderResolver.fillPlaceholders(content)(session, rc, noExtractor) == Right(s"3.15 > $value")
  }

  property("fillPlaceholders returns ResolverError for the first placeholder not found") = forAll(keyGen, valueGen, keyGen, valueGen) { (k1, v1, k2, v2) =>
    val session = Session.newEmpty.addValuesUnsafe(k1 -> v1, k2 -> v2)
  ...
  property("fillPlaceholders generates random uuid if <random-uuid> - fixed by seed") = forAll { (seed: Long) =>
  ...
  property("fillPlaceholders generates random positive integer if <random-positive-integer> - fixed by seed") = forAll { (seed: Long) =>
    val session = Session.newEmpty
  ...
  property("fillPlaceholders generates random string if <random-string> - fixed by seed") = forAll { (seed: Long) =>
    val session = Session.newEmpty
  ...
  property("fillPlaceholders generates random alphanumeric string if <random-alphanum-string> - fixed by seed") = forAll { (seed: Long) =>
    val session = Session.newEmpty
  ...
  property("fillPlaceholders generates random boolean if <random-boolean> - fixed by seed") = forAll { (seed: Long) =>
    val session = Session.newEmpty
  ...
  property("fillPlaceholders generates random timestamp string if <random-timestamp>") = forAll { (seed: Long) =>
    val session = Session.newEmpty
  ...
  property("fillPlaceholders generate current timestamp string if <current-timestamp>") = forAll { (seed: Long) =>
    val session = Session.newEmpty
