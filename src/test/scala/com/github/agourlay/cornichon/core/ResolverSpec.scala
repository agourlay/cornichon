...
class ResolverSpec extends WordSpec with Matchers {
  ...
  "A resolver" must {
    "replace a single string" in {
      ...
      resolver.fillPlaceholders(content)(source) should be(right("This project is named cornichon"))
    }

    "not be confused by markup order" in {
      val source = new HashMap[String, String].+("project-name" → "cornichon")
      val content = "This project is named >project-name<"
      resolver.fillPlaceholders(content)(source) should be(right("This project is named >project-name<"))
    }
    ...
    "return ResolverError if placeholder not found" in {
      ...
      resolver.fillPlaceholders(content)(source) should be(left(ResolverError("project-new-name")))
    }
    ...
    "replace two strings" in {
      ...
      resolver.fillPlaceholders(content)(source) should be(right("This project is named cornichon and is super tasty"))
    }
    ...
    "return ResolverError for the first placeholder not found" in {
      ...
      resolver.fillPlaceholders(content)(source) should be(left(ResolverError("new-taste")))
    }

    "generate random-uuid" in {
      val source = HashMap.empty[String, String]
      val content = "<random-uuid>"
      resolver.fillPlaceholders(content)(source) should not be (right("<random-uuid>"))
    }
  }
}
