...
class Engine {
      ...
      if (steps.isEmpty)
        ...
        steps.head match {
          case DebugStep(message) ⇒
            loop(steps.tail, session, eventuallyConf, snapshot, logs :+ ColoredLogInstruction(message(session), CYAN))
  ...
  private def buildFailedScenarioReport(scenario: Scenario, steps: Seq[Step], currentStep: ExecutableStep[_], e: CornichonError, logs: Seq[LogInstruction]): FailedScenarioReport = {
