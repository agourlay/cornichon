...
case class CollectionNotEmptyAssertion[A: Show](collection: Iterable[A]) extends CollectionAssertion[A] {
...
case class CollectionNotEmptyAssertionError[A: Show](collection: Iterable[A]) extends CornichonError {
  val msg = s"collection expected to be non empty"
}

case class CollectionEmptyAssertion[A: Show](collection: Iterable[A]) extends CollectionAssertion[A] {
  val validated: ValidatedNel[CornichonError, Done] =
    if (collection.isEmpty) valid(Done) else invalidNel(CollectionEmptyAssertionError(collection))
}

case class CollectionEmptyAssertionError[A: Show](collection: Iterable[A]) extends CornichonError {
  val msg = s"expected collection to be empty but it contains:\n${collection.show}"
}

case class CollectionSizeAssertion[A: Show](collection: Iterable[A], size: Int) extends CollectionAssertion[A] {
  val validated: ValidatedNel[CornichonError, Done] =
