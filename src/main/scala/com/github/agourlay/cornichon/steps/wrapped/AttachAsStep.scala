  ...
  def run(engine: Engine, session: Session, depth: Int)(implicit ec: ExecutionContext) = {
    ...
    res match {
      case s: SuccessStepsResult ⇒
        ...
        (newSession, s.copy(logs = updatedLogs))
      case f: FailureStepsResult ⇒
        val failureLogs = failedTitleLog(depth) +: f.logs :+ FailureLogInstruction(s"'$title' failed", depth)
        (newSession, f.copy(logs = failureLogs))
    }
  }
}
